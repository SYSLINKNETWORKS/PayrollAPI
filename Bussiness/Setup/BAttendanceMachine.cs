...
{
    public class BAttendanceMachine : AbsBusiness
    {
        ...
        public BAttendanceMachine(DataContext context)
        {
            _context = context;
        }
        ...
        public override async Task<ApiResponse> GetDataAsync(ClaimsPrincipal _User)
        {
            ...
            try
            {
                var _Table = await _context.AttendanceMachines.Include(x => x.attendanceMachineCategory).Include(g => g.attendanceMachineGroup).Where(a => a.Action != Enums.Operations.D.ToString()).OrderBy(o => o.Name).ToListAsync();
        ...
        public override async Task<ApiResponse> GetDataByIdAsync(Guid _Id, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                var _Table = await _context.AttendanceMachines.Include(x => x.attendanceMachineCategory).Include(g => g.attendanceMachineGroup).Where(a => a.Id == _Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefaultAsync();
        ...
        public override async Task<ApiResponse> AddAsync(object model, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                _UserName = _User.Claims.FirstOrDefault(c => c.Type == Enums.Misc.UserName.ToString())?.Value.ToString();
                var _model = (AttendanceMachine)model;

                string error = "";
                bool _NameExists = _context.AttendanceMachines.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.Action != Enums.Operations.D.ToString());
                ...
                await _context.AttendanceMachines.AddAsync(_model);
        ...
        public override async Task<ApiResponse> UpdateAsync(object model, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                string error = "";
                bool _NameExists = _context.AttendanceMachines.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.Id != _model.Id && rec.Action != Enums.Operations.D.ToString());
                ...
                var result = _context.AttendanceMachines.Where(a => a.Id == _model.Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefault();
                if (result == null)
                {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString();
                    ApiResponse.message = "Record not found ";
                ...
                result.AttendanceMachineCategoryId = _model.AttendanceMachineCategoryId;
                result.AttendanceMachineGroupId = _model.AttendanceMachineGroupId;
        ...
        public override async Task<ApiResponse> DeleteAsync(Guid _Id, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                _UserName = _User.Claims.FirstOrDefault(c => c.Type == Enums.Misc.UserName.ToString())?.Value.ToString();
                var _Table = _context.AttendanceMachines.Where(a => a.Id == _Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefault();
                if (_Table == null)
                {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString();
                    ApiResponse.message = "Record not found ";
