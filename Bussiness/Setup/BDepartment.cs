...
namespace TWP_API_Payroll.Bussiness {
    public class BDepartment : AbsBusiness {
        private readonly DataContext _context;
        private ErrorLog _ErrorLog = new ErrorLog ();

        String _UserName = "";

        public BDepartment (DataContext context) {
            _context = context;
        }
        ...
        public override async Task<ApiResponse> GetDataAsync (ClaimsPrincipal _User) {
            ...
            try {
                var _Table = await _context.Departments.Where (a => a.Action != Enums.Operations.D.ToString ()).OrderBy (o => o.Name).ToListAsync ();
        ...
        public override async Task<ApiResponse> GetDataByIdAsync (Guid _Id, ClaimsPrincipal _User) {
            ...
            try {
                ...
                var _Table = await _context.Departments.Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefaultAsync ();
        ...
        public override async Task<ApiResponse> AddAsync (object model, ClaimsPrincipal _User) {
            ...
            try {
                ...
                await _context.Departments.AddAsync (_model);
        ...
        public override async Task<ApiResponse> UpdateAsync (object model, ClaimsPrincipal _User) {
            ...
            try {
                ...
                string error = "";
                bool _NameExists = _context.Departments.Any (rec => rec.Name.Trim ().ToLower ().Equals (_model.Name.Trim ().ToLower ()) && rec.Id!=_model.Id && rec.Action != Enums.Operations.D.ToString ());
                ...
                var result = _context.Departments.Where (a => a.Id == _model.Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (result == null) {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    ApiResponse.message = "Record not found ";
                    return ApiResponse;
        ...
        public override async Task<ApiResponse> DeleteAsync (Guid _Id, ClaimsPrincipal _User) {
            ...
            try {
                ...
                _UserName = _User.Claims.FirstOrDefault (c => c.Type == Enums.Misc.UserName.ToString ())?.Value.ToString ();
                var _Table = _context.Departments.Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (_Table == null) {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    ApiResponse.message = "Record not found ";
                    return ApiResponse;
