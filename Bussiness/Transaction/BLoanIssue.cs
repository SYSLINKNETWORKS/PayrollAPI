...
{
    public class BLoanIssue : AbsBusiness
    {
        private readonly DataContext _context;
        private ErrorLog _ErrorLog = new ErrorLog();

        String _UserName = "";
        MSVoucher _MSVoucher = new MSVoucher();

        public BLoanIssue(DataContext context)
        {
            _context = context;
        }
        ...
        public override async Task<ApiResponse> GetDataAsync(ClaimsPrincipal _User)
        {
            ...
            try
            {
                var _Table = await _context.LoanIssues.Include(emp => emp.Employee).Include(loan => loan.LoanCategory).Where(a => a.Action != Enums.Operations.D.ToString()).OrderBy(o => o.Id).ToListAsync();
        ...
        public override async Task<ApiResponse> GetDataByIdAsync(Guid _Id, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                var _Table = await _context.LoanIssues.Include(emp => emp.Employee).Include(loan => loan.LoanCategory).Where(a => a.Id == _Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefaultAsync();
        ...
        public override async Task<ApiResponse> AddAsync(object model, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                var _model = (LoanIssue)model;


                string error = "";
                bool _NameExists = _context.LoanIssues.Any(rec => rec.Date.Equals(_model.Date) && rec.EmployeeId.Equals(_model.EmployeeId) && rec.Action != Enums.Operations.D.ToString());
                ...
                // //Voucher
                // string _Narration = "Paid to " + _EmployeeName + " against loan (Deducation " + _model.InstalmentAmount.ToString() + "/month from " + _model.Date.ToString("dd-MMM-yyyy") + ")";
                ...
                // var _VoucherApiResponse = await _MSVoucher.AddLoanVoucher(_Voucher, _UserKey);
                ...
                //     await _context.LoanIssues.AddAsync(_model);
        ...
        public override async Task<ApiResponse> UpdateAsync(object model, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                // var _VoucherApiResponse = await _MSVoucher.UpdateLoanVoucher(_Voucher, _UserKey);
                // if (_VoucherApiResponse.statusCode != StatusCodes.Status200OK.ToString()) { return _VoucherApiResponse; }
                // var _VoucherResponse = (VoucherResponse)_VoucherApiResponse.data;
                // if (_VoucherApiResponse.statusCode != StatusCodes.Status200OK.ToString())
                // {
        ...
        public override async Task<ApiResponse> DeleteAsync(Guid _Id, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                var _Table = _context.LoanIssues.Where(a => a.Id == _Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefault();
                if (_Table == null)
                {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString();
                    ApiResponse.message = "Record not found ";
                ...
                // var _VoucherApiResponse = await _MSVoucher.DeleteLoanVoucher(_Table.VoucherNo, _CompanyId, _UserId, _UserName, _UserKey);
