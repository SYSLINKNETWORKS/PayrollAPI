...
namespace TWP_API_Payroll.Processor.Process.Payroll {
    public class AttendanceMachineGroupProcessor : IProcessor<AttendanceMachineGroupBaseModel> {
        private DataContext _context;
        private AbsBusiness _AbsBusiness;
        private SecurityHelper _SecurityHelper = new SecurityHelper ();

        public AttendanceMachineGroupProcessor (App_Data.DataContext context) {
            _context = context;

            _AbsBusiness = Builder.MakeBusinessClass (Enums.ClassName.AttendanceMachineGroup, _context);
        }
        public async Task<ApiResponse> ProcessGet (Guid _MenuId, ClaimsPrincipal _User) {
            ...
            if (_AbsBusiness != null) {
                ...
                if (Convert.ToInt32 (response.statusCode) == 200) {
                    var _Table = (IEnumerable<AttendanceMachineGroup>) response.data;
                    var result = (from ViewTable in _Table select new AttendanceMachineGroupViewModel {
                        Id = ViewTable.Id,
                            Name = ViewTable.Name,
                            Type = ViewTable.Type,
        ...
        public async Task<ApiResponse> ProcessGetById (Guid _Id, Guid _MenuId, ClaimsPrincipal _User) {
            ...
            if (_AbsBusiness != null) {
                ...
                if (Convert.ToInt32 (response.statusCode) == 200) {
                    var _Table = (AttendanceMachineGroup) response.data;
                    var _ViewModel = new AttendanceMachineGroupViewByIdModel {
                        Id = _Table.Id,
                        Name = _Table.Name,
                        Type = _Table.Type,
        ...
        public async Task<ApiResponse> ProcessPost (object request, ClaimsPrincipal _User) {
            ApiResponse apiResponse = new ApiResponse ();
            if (_AbsBusiness != null) {
                var _request = (AttendanceMachineGroupAddModel) request;
                ...
                var _Table = new AttendanceMachineGroup {
                    Name = _request.Name,
        ...
        public async Task<ApiResponse> ProcessPut (object request, ClaimsPrincipal _User) {
            ...
            ApiResponse apiResponse = new ApiResponse ();
            if (_AbsBusiness != null) {
                var _request = (AttendanceMachineGroupUpdateModel) request;
        ...
        public async Task<ApiResponse> ProcessDelete (object request, ClaimsPrincipal _User) {
            ApiResponse apiResponse = new ApiResponse ();
            if (_AbsBusiness != null) {
                var _request = (AttendanceMachineGroupDeleteModel) request;
