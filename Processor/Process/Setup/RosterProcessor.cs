...
namespace TWP_API_Payroll.Processor.Process.Payroll {
    public class RosterProcessor : IProcessor<RosterBaseModel> {
        private DataContext _context;
        private AbsBusiness _AbsBusiness;
        private SecurityHelper _SecurityHelper = new SecurityHelper ();

        public RosterProcessor (App_Data.DataContext context) {
            _context = context;

            _AbsBusiness = Builder.MakeBusinessClass (Enums.ClassName.Roster, _context);
        }
        public async Task<ApiResponse> ProcessGet (Guid _MenuId, ClaimsPrincipal _User) {
            ...
            if (_AbsBusiness != null) {
                ...
                if (Convert.ToInt32 (response.statusCode) == 200) {
                    var _Table = (IEnumerable<Roster>) response.data;
                    var result = (from ViewTable in _Table select new RosterViewModel {
                        Id = ViewTable.Id,
                            Name = ViewTable.Name,
                            Type = ViewTable.Type,
        ...
        public async Task<ApiResponse> ProcessGetById (Guid _Id, Guid _MenuId, ClaimsPrincipal _User) {
            ...
            if (_AbsBusiness != null) {
                ...
                if (Convert.ToInt32 (response.statusCode) == 200) {
                    var _Table = (Roster) response.data;
                    var _ViewModel = new RosterViewByIdModel {
                        Id = _Table.Id,
                        Name = _Table.Name,
                        Type = _Table.Type,
        ...
        public async Task<ApiResponse> ProcessPost (object request, ClaimsPrincipal _User) {
            ApiResponse apiResponse = new ApiResponse ();
            if (_AbsBusiness != null) {
                var _request = (RosterAddModel) request;
        ...
        public async Task<ApiResponse> ProcessPut (object request, ClaimsPrincipal _User) {
            ...
            ApiResponse apiResponse = new ApiResponse ();
            if (_AbsBusiness != null) {
                var _request = (RosterUpdateModel) request;
        ...
        public async Task<ApiResponse> ProcessDelete (object request, ClaimsPrincipal _User) {
            ApiResponse apiResponse = new ApiResponse ();
            if (_AbsBusiness != null) {
                var _request = (RosterDeleteModel) request;
